...
def main():
    ...
    # D. TOPSIS Veri Seti
    cols_needed = ['encounter_id', 'patient_nbr', 'time_in_hospital', 'number_emergency', 'num_medications',
                   'num_procedures']
    df_topsis = df_test_original[cols_needed].copy()
    ...
    # --- AHP ENTEGRASYONU ---
    # Kriterler: [Model Riski, Acil Servis, Hastane Süresi, İşlem Sayısı]
    criteria = ['xgboost_risk_score', 'number_emergency', 'time_in_hospital', 'num_procedures']

    # AHP'den gelen bilimsel ağırlıklar (CR=0.0138)
    weights = [0.5555, 0.2513, 0.0966, 0.0966]
